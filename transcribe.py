...
INITIAL_ROTATION_VALUES = [90, 90, 90, 90, 65]
...
def pix_send1(event, rotation_angle):
    time.sleep(0.03)
    Adeept.three_function("'servo_write'",0, rotation_angle)
    #print(var_A.get())
def pix_send2(event, rotation_angle):
    time.sleep(0.03)
...
def pix_send4(event, rotation_angle):
    time.sleep(0.03)
    Adeept.three_function("'servo_write'",3, rotation_angle)
    #print(var_D.get())
def pix_send5(event, rotation_angle):
    time.sleep(0.03)
    Adeept.three_function("'servo_write'",4, rotation_angle)
    #print(var_E.get())
...
def on_message(self, msg):
    ...
    if "results" in data:
        ...
        message = data['results'][0]['alternatives'][0]['transcript'].lower()
        execute_instruction(message)

def execute_instruction(message):
    print(message)
    print(type(message))

    if message == "pathfinder" or "pathfinder" in message:
            INITIAL_ROTATION_VALUES[2] = 60
            pix_send3(None, INITIAL_ROTATION_VALUES[2])
        
    elif message == "down" or "down" in message:
        if INITIAL_ROTATION_VALUES[2] <= 150:
            INITIAL_ROTATION_VALUES[2] += 30
            pix_send3(None, INITIAL_ROTATION_VALUES[2])

    elif message == "up" or "up" in message:
        print("GOING UP! Using Grapple!")
        if INITIAL_ROTATION_VALUES[2] >= 30:
            INITIAL_ROTATION_VALUES[2] -= 30
            pix_send3(None, INITIAL_ROTATION_VALUES[2])
